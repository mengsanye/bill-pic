...
完整设计规范见 **[`design/icon/ICON-SPEC.md`](design/icon/ICON-SPEC.md)**。

**概念（v2）**：「一页 PDF，出来一张图」—— 白色纸张 + 右上折角 + 三条文字条 = PDF 文档，
占画面主导；右下角滑出的一块**带白边的方形照片**（内含缩小的景物）= 转换出来的图片。

**为什么要改**：v1 是「纸张里放远山、近山、太阳」。而**"白底 + 满幅风景"正是所有相册 / 图库
App 的构图**，所以它在桌面上一眼被归到「图库」那一类，「转换」的语义完全丢失。
重新设计时用脚本渲染了 22 个候选逐一对照，得到三条硬规律（已写进 `generate_icons.py` 与规范文档）：

| 尝试 | 结果 | 原因 |
|---|---|---|
| 蓝白**对角**切分 | ✗ 读成"山" | 白色在上、蓝色三角在下＝山的原型。**只要出现三角分界就必然如此** |
| 纯色圆角方块代表"图片" | ✗ 读成形状 / 按钮 / 贴纸 | 方块本身没有"图"的语义 |
| 两张同形白纸错位 | ✗ 读成"复制 / 两个文件" | 同材质并置＝同一类事物 |
| 取景框四角 | ✗ 读成"扫描 / 裁剪工具" | 语义偏向相机 |
| **文档 + 从它出来的带白边照片** | ✓ | 白框是**印刷照片**的通用符号；风景只在照片内部 |
...
| 可复现脚本 | `design/icon/generate_icons.py`（含几何自检 + 40px/dp 超采样） |

**关键取舍**：纸张 32×45 dp（比例 0.711 ≈ A4 的 0.707），照片块向右溢出 5 dp、向下 3 dp
—— 溢出是"从纸里出来"的关键；为此纸张比 v1 缩小了一档，代价已在 48px 降级图中确认可控。
前景最远点 **31.28 dp < 33 dp** 安全界（脚本自检通过）。legacy 位图**裁中央 72dp** 生成，
保证新旧设备观测一致。

**两个几何陷阱**（都写在规范里，后续改动别踩）：
1. **单色层不能改成一刀切的减法**：照片块与纸张重叠，直接"纸张实心 − 照片"会把照片一起挖掉。
   用**奇偶填充**（evenOdd，位图侧 `ImageChops.logical_xor`）一次做出四层关系。
2. **照片内容要裁剪**：山体底边落在内框下沿，不裁剪三角尖会戳出圆角。
   位图与矢量用**同一套圆角遮罩**（矢量侧 `<clip-path>`）。

**已实测**：圆形蒙版下折角 / 文字条 / 相框 / 卡纸 / 双峰 / 太阳全部正确、无裁切；
与 Phone / Messages / Chrome 并排清晰可辨，**不再被归入图库类**；48/72/96/144/192 五档降级均成立。
